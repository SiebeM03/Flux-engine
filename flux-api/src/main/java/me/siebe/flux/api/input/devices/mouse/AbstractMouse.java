...
public abstract class AbstractMouse implements Mouse {
    ...
    /** Buttons that were pressed this frame (cleared in {@link #endFrame()}). */
    private final BitSet buttonsPressedThisFrame;
    /** Buttons that were released this frame (cleared in {@link #endFrame()}). */
    ...
    /**
     * Latest normalized position from move callbacks; committed to {@link #x} in {@link #beginFrame()}.
     */
    private float pendingNormalizedX;
    /**
     * Latest normalized position from move callbacks; committed to {@link #y} in {@link #beginFrame()}.
     */
    private float pendingNormalizedY;

    /**
     * Horizontal scroll offset this frame. Reset in {@link #endFrame()}.
     * Magnitude and sign depend on the input device (mouse wheel vs trackpad) and platform — see the class documentation above.
     */
    private float scrollX;
    /**
     * Vertical scroll offset this frame: positive = scroll up, negative = scroll down. Reset in {@link #endFrame()}.
     * Magnitude and sign depend on the input device (mouse wheel vs trackpad) and platform — see the class documentation above.
     */
    ...
    @Override
    public float deltaY() {
        return normalizedDeltaY() * AppContext.get().getWindow().getHeight();
    }
     ...
     * {@inheritDoc}
     * <p>
     * Clears per-frame button and scroll state.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Commits pending position to {@link #x}/{@link #y}
     */
    @Override
     ...
     * they are committed in {@link #beginFrame()}. Resets double-click detection so a second click only
     ...
     * Values are stored for the current frame and reset in {@link #endFrame()}.
     * <p>
